...
# Rempli le dicionnaire avec des requêtes à la db pour appeler la fonction produce_pdf
    ...
    dict_pdf["theme"] = db.session.query(Theme).filter(Theme.id == id_theme)[0].name
...
def evaluationpdf(id,time):
    ...
    ari = ""
    for item in range(5):
        name = db.session.query(Chapitre).filter(Chapitre.id == item+1)[0].name
        note1 = db.session.query(Note).filter(Note.id_eleve == id).\
            filter(Note.id_item == item+1).filter(Note.niveau == 1).all()[0].note
        note2 = db.session.query(Note).filter(Note.id_eleve == id).\
    ...
    for item in range(4):
        name =db.session.query(Chapitre).filter(Chapitre.id == item + 6)[0].name
        note1 = db.session.query(Note).filter(Note.id_eleve == id).\
    ...
    for item in range(3):
        name =db.session.query(Chapitre).filter(Chapitre.id == item + 10)[0].name
        note1 = db.session.query(Note).filter(Note.id_eleve == id).\
    ...
    for item in range(4):
        name =db.session.query(Chapitre).filter(Chapitre.id == item + 13)[0].name
        note1 = db.session.query(Note).filter(Note.id_eleve == id).\
...
def note(id):
    ...
    Dict_eleve = {}
    Dict_eleve["id"] = id
    Dict_eleve["nom"] = db.session.query(Eleves).filter(Eleves.ID == Dict_eleve["id"])[0].Nom
    Dict_eleve["prenom"] = db.session.query(Eleves).filter(Eleves.ID == Dict_eleve["id"])[0].Prenom
